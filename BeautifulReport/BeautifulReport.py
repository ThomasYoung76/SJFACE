...
@File: __init__.py.py
@License: MIT
@Note: 2018/02/09 modify ReportTestResult.stopTest by ysf to change result_list. the content of "用例描述" has been changed
"""
...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTest(self, test) -> None:
        ...
        # self.result_list.append(self.get_all_result_info_tuple(test)) # 原始代码
        case_result_info = self.get_all_result_info_tuple(test) # added by ysf
        case_result_info = list(case_result_info)   # added by ysf
        case_desc = case_result_info[2] # added by ysf
        case_name = re.findall("case_name=\\'(.*?)\\'", case_desc) # added by ysf
        try:
            case_result_info[2] = case_name[0] # added by ysf
        except IndexError:
            print("用例名不存在，请检查用例名")
            sys.exit(1)
        case_result_info = tuple(case_result_info)  # added by ysf
        self.result_list.append(case_result_info) # added by ysf
